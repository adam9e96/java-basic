...
public class Array_09_HowToMerge {
    public static void main(String[] args) {
        ...
        int[] array1 = {1, 2, 3};
        int[] array2 = {4, 5, 6};
        int[] mergedArray = new int[array1.length + array2.length]; // 6
        System.out.println("test: " + mergedArray.length); // test: 6


        for (int i = 0; i < array1.length; i++) {
            mergedArray[i] = array1[i]; // 0 1 2 인덱스 값 초기화
        }

        for (int i = 0; i < array2.length; i++) {
            mergedArray[array1.length + i] = array2[i]; // 3 4 5 인덱스 값 초기화
        }
